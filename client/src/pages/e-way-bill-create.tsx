...
export default function EWayBillCreate() {
    ...
    const fetchInitialData = async () => {
        try {
            const [custRes, cnRes, invRes, dcRes, soRes] = await Promise.all([
                fetch('/api/customers'),
                fetch('/api/credit-notes'),
                fetch('/api/invoices'),
                fetch('/api/delivery-challans'),
                fetch('/api/sales-orders')
            ]);

            if (custRes.ok) setCustomers((await custRes.json()).data || []);
            if (cnRes.ok) setCreditNotes((await cnRes.json()).data || []);
            if (invRes.ok) setInvoices((await invRes.json()).data || []);
            if (dcRes.ok) setDeliveryChallans((await dcRes.json()).data || []);
            if (soRes.ok) setSalesOrders((await soRes.json()).data || []);
        } catch (error) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (invoice.billingAddress) {
                    setAddressData(prev => ({
                        ...prev,
                        billTo: { ...prev.billTo, ...invoice.billingAddress }
                    }));
                }
                if (invoice.items) setSelectedItems(invoice.items);
            }
        } catch (error) {
    ...
    const fetchChallanForEWayBill = async (challanId: string) => {
        try {
            ...
            if (response.ok) {
                ...
                if (challan.items) setSelectedItems(challan.items);
            }
        } catch (error) {
    ...
    const handleSave = async (generate: boolean = false) => {
        ...
        try {
            const payload = {
                ...formData,
                ...addressData,
                status: generate ? 'GENERATED' : 'NOT_GENERATED',
            };
            ...
            if (response.ok) {
                toast({
                    title: generate ? "E-Way Bill Generated" : "E-Way Bill Saved",
                    description: `E-way bill has been ${generate ? 'generated' : 'saved'} successfully.`,
                });
        ...
        <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
            ...
            <div className="flex-none bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-900">New e-Way Bill</h1>
                        {fromInvoiceNumber && (
                            <p className="text-xs text-blue-600 font-medium">From Invoice #{fromInvoiceNumber}</p>
                        )}
                    </div>
                </div>
